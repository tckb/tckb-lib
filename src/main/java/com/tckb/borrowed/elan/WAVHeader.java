package com.tckb.borrowed.elan;
...
public final class WAVHeader {
    ...
    public String getInfo() {
        ...
        for (Iterator iter = infos.keySet().iterator(); iter.hasNext();) {
            ...
            info.append("\n").append(key).append(" : ").append(infos.get(key));
        }
    ...
    public String toString() {
        ...
        try {
            ...
            for (int i = 0; i < formatSpecific.length; i++) {
                output.append("\n").append(36 + (i * 2)).append("-").append(37 + (i * 2));
                output.append(" Format specific data    : ").append(formatSpecific[i]);
            }
            //HS 07-2011 added info from "fact" chunk in header
            if (factID != null) {
            	output.append("\n================ fact chunk =================");
                output.append("\n").append(20 + fLen).append("-").append(23 + fLen).append(" Letters                 : ").append(factID);
                output.append("\n").append(24 + fLen).append("-").append(27 + fLen).append(" Length of rest of chunk : ").append(factLen);
                output.append("\n").append(28 + fLen).append("-").append(31 + fLen).append(" dwSampleLength          : ").append(dwSampleLength);
            }
            
            output.append("\n================ data chunk =================");
            output.append("\n").append(20 + fLen + factLen).append("-").append(23 + fLen + factLen).append(" Letters                 : ");
            output.append(dID).append("\n").append(24 + fLen + factLen).append("-").append(27 + fLen + factLen);
            output.append(" Length of following data: ").append(dLen).append("\n").append(28 + fLen + factLen).append("-")
                  .append(28 + fLen + factLen + dLen)
                  .append(" (data)");
            ...
            if (cueSections.length > 0) {
                ...
                for (int i = 0; i < cueSections.length; i++) {
                    output.append("\nCue section ").append(i).append(":\n").append(cueSections[i]);
                }
            }
    ...
    private void readAssociatedDataList(RandomAccessFile soundFile) {
        ...
        try {
            ...
            cueSections = (WAVCueSection[]) cueSectionList.toArray(new WAVCueSection[cueSectionList.size()]);
        } catch (IOException e) {
