...
package com.tckb.borrowed.tarsos.fft;
...
public strictfp class FloatFFT {
    ...
    public FloatFFT(int n) {
        ...
        if (!FFTUtils.isPowerOf2(n)) {
            if (getReminder(n, factors) >= 211) {
                ...
                nBluestein = FFTUtils.nextPow2(n * 2 - 1);
    ...
    public void realForwardFull(final float[] a, final int offa) {
        ...
        case SPLIT_RADIX:
            realForward(a, offa);
            int nthreads = FFTUtils.getNumberOfThreads();
            if ((nthreads > 1) && (n / 2 > FFTUtils.getThreadsBeginN_1D_FFT_2Threads())) {
                Future<?>[] futures = new Future[nthreads];
                int k = n / 2 / nthreads;
                for (int i = 0; i < nthreads; i++) {
                    final int firstIdx = i * k;
                    final int lastIdx = (i == (nthreads - 1)) ? n / 2 : firstIdx + k;
                    futures[i] = FFTUtils.submit(new Runnable() {
                        public void run() {
                            int idx1, idx2;
                ...
                FFTUtils.waitForCompletion(futures);
            } else {
                int idx1, idx2;
                for (int k = 0; k < n / 2; k++) {
                    idx1 = 2 * k;
    ...
    private void bluestein_complex(final float[] a, final int offa, final int isign) {
        ...
        int nthreads = FFTUtils.getNumberOfThreads();
        if ((nthreads > 1) && (n > FFTUtils.getThreadsBeginN_1D_FFT_2Threads())) {
            nthreads = 2;
            if ((nthreads >= 4) && (n > FFTUtils.getThreadsBeginN_1D_FFT_4Threads())) {
                nthreads = 4;
            }
            Future<?>[] futures = new Future[nthreads];
            int k = n / nthreads;
    ...
    private void cftfsub(int n, float[] a, int offa, int[] ip, int nw, float[] w) {
        if (n > 8) {
            if (n > 32) {
                ...
                if ((FFTUtils.getNumberOfThreads() > 1) && (n > FFTUtils.getThreadsBeginN_1D_FFT_2Threads())) {
    ...
    private void cftrec4_th(final int n, final float[] a, final int offa, final int nw, final float[] w) {
        ...
        for (i = 0; i < nthreads; i++) {
            ...
            if (i != idiv4) {
                futures[idx++] = FFTUtils.submit(new Runnable() {
                    public void run() {
                        int isplt, j, k, m;
    ...
    private void scale(final float m, final float[] a, int offa, boolean complex) {
        ...
        int nthreads = FFTUtils.getNumberOfThreads();
        if ((nthreads > 1) && (n2 >= FFTUtils.getThreadsBeginN_1D_FFT_2Threads())) {
